...
Use `/agent splinter` to analyse and dispatch the right turtle(s) for any non-trivial task.

| Agent | Role |
|-------|------|
| `splinter` | Orchestrator — picks and dispatches turtles |
| `turtleman` | Entry point — activates mode, routes to splinter |
...
Use `/fleet` to dispatch multiple turtles in parallel.
